...
async def send_message(
    ...
    Args:
        ...
        reply_to: Optional message ID to reply to. For supergroups in forum/topic
            mode, pass the topic ID here (which equals the root message ID of that
            thread) so the message lands in that topic instead of General.
    """
    try:
        cl = get_client(account)
        entity = await resolve_entity(chat_id, cl)
        await cl.send_message(entity, message, parse_mode=parse_mode, reply_to=reply_to)
...
async def forward_message(
    ...
    Args:
        ...
        reply_to: Optional forum topic ID (which equals the root message ID of
            that thread) to forward into. For supergroups in forum/topic mode,
            pass the topic ID here so the forwarded message lands in that topic
            instead of General.
    """
    try:
        cl = get_client(account)
        ...
        if reply_to is not None:
            # The high-level forward_messages helper can't target a forum topic,
            # so use the raw request with top_msg_id to route into the thread.
            await cl(
